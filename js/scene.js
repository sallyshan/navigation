...
swiper.prototype = {
	...
	_initDom : function(){
		...
		for(var i = 0; i < self._item.length; i++){
			...
			$(self._item[i]).css("-webkit-transform","translate3d(0,"+i*self._width+"px,0)");

			self.btn && self.btn.on("tap",function(){
	...
	_initEvent : function(){
		...
		this._item.on("touchstart",function(e){
			...
			self._startx = e.touches[0].screenY;
		...
		this._item.on("touchmove",function(e){
			...
			self._movex = e.touches[0].screenY;
	...
	_start:function(num,type){
		...
		//pre page
		item[newnum-1] && ($(item[newnum-1]).css({"-webkit-transform":"translate3d(0,"+-w+"px,0)","-webkit-transition":"transform 0.3s ease-in"}));
